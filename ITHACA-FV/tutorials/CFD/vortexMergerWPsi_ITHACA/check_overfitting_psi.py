...
snap_trial_psi = np.load("snap_trial_psi0_20.npy")
param_trial = np.load("param_trial0_20.npy")
snap_test_psi = np.array(np.load("snap_test_psi0_20.npy"))
...
def train(**kwargs):
    ...
    X_train, X_test, Y_train, Y_test = param_trial, param_test, snap_trial_psi, snap_test_psi
    ...
    np.save(Net.hash()+"_trainLossPPPSI0_20.npy", lossplottrain)
    np.save(Net.hash()+"_testLossPPPSI0_20.npy",lossplottest)
    m.save(Net.hash()+".pt")
...
if __name__ == '__main__':
    ...
    target_data = np.load("snap_trial_psi0_20.npy")
    ...
    train(input_data=input_data,
          ...
          nn=Net, learning_rate=1e-5, epochs=20000)