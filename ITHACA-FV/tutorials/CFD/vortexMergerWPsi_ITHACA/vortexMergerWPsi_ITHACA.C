...
class vortexMergerWPsi_ITHACA: public unsteadyNSWPsi
{
    public:
        explicit vortexMergerWPsi_ITHACA(int argc, char* argv[])
            :
            unsteadyNSWPsi(argc, argv),
            W(_W()),
            U(_U()),
            Psi_z(_Psi_z())
        {}
        ...
        void offlineSolve()
        {
            ...
            if (offline)
            {
                ITHACAstream::read_fields(Wfield, W, "./ITHACAoutput/Offline/");
                ITHACAstream::read_fields(Psi_zfield, Psi_z, "./ITHACAoutput/Offline/");
            }
            else
            {
...
int main(int argc, char* argv[])
{
    ...
    vortexMergerWPsi_ITHACA example(argc, argv);
    ...
    ITHACAPOD::getModes(example.Wfield, example.Wmodes, example._W().name(),
                        example.podex, 0, 0,
                        NmodesWout);
