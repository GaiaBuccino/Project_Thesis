...
#include "ReducedUnsteadyNSWPsi.H"


// * * * * * * * * * * * * * * * Constructors * * * * * * * * * * * * * * * * //

// Constructor initialization
reducedUnsteadyNSWPsi::reducedUnsteadyNSWPsi()
{
}
    ...
    int startSnap)
{
    ...
    Eigen::MatrixXd tmp_sol(Nphi_w + Nphi_psi_z + 1, 1);
    tmp_sol(0) = time;
...
*/
void reducedUnsteadyNSWPsi::reconstruct(bool exportFields, fileName folder)
{
    if (exportFields)
    {
        mkDir(folder);
    ...
    for (int i = 0; i < online_solution.size(); i++)
    {
        if (counter == nextwrite)
        {
            ...
            Eigen::MatrixXd currentPsi_zCoeff;
            currentWCoeff = online_solution[i].block(1, 0, Nphi_w, 1);
            currentPsi_zCoeff = online_solution[i].bottomRows(Nphi_psi_z);
            CoeffW.append(currentWCoeff);
            CoeffPsi_z.append(currentPsi_zCoeff);
    ...
    volScalarField wRec("wRec", Wmodes[0] * 0);
    volScalarField psi_zRec("psi_zRec", problem->Psi_zmodes[0] * 0);
    wRecFields = problem->Wmodes.reconstruct(wRec, CoeffW, "wRec");
    psi_zRecFields = problem->Psi_zmodes.reconstruct(psi_zRec, CoeffPsi_z, "psi_zRec");
